...
class ApiDataObject extends DataExtension
{
  ...
  public static function to_underscore($input)
  {
    ...
    public static function to_camelcase($input, $checkID = true)
    {
        $s = "";
   ...
   */
  public static function to_nested_array($object, $level = 0, &$data)
  {
      if ($level > 5) {
          return null;
      }
   ...
   * @return array fields
   */
  public function inheritedApiFields()
  {
      $fields     = array();
   ...
   */
  public static function real_field_name($field, $ownerClass)
  {
      ...
      } elseif (preg_match("/^[a-z\_0-9]+$/", $field)) {
   ...
   */
  public function forApi($options = null)
  {
    ...
    foreach ($fields as $key => $k) {
      ...
      if ($value === null) {
          // keep as null
        $data[$k] = null;
      } elseif ($fieldType === 'int' || $fieldType === 'primarykey') {
          $data[$k] = (int) $value;
      } elseif ($fieldType === 'float' || $fieldType === 'decimal' || $fieldType === 'currency' || $fieldType === 'percentage') {
          $data[$k] = (float) $value;
      ...
      } elseif ($fieldType === 'ss_datetime' || $fieldType === 'date' || $fieldType === 'time') {
          $data[$k] = $this->owner->dbObject($key)->Format($jsonDateFormat);
      } elseif (is_a($value, 'DataObject')) {
   ...
   */
  public function populateWithData($data, $only = null, $exclude = null)
  {
    ...
    public function populateWithArrayData(array $data, $only = null, $exclude = null)
    {
