...
class ApiController extends Controller
{
  ...
  protected function setSessionByApiSession()
  {
    ...
    } elseif ($this->config()->get('useAccesstokenExclusivelyAsAuthentication')) {
        ...
        foreach ($apiParameters as $methodAndAction => $parameters) {
          ...
          $isRequired = ($field[strlen($field)-1] === '!') ? true : false;
                    ...
                    if ($isQueryParameter) {
                        ...
                        } elseif (($field !== $camelCaseFieldName) && (isset($requestVars[$camelCaseFieldName]))) {
                            $value = $requestVars[$camelCaseFieldName];
                        }
                    } elseif ($isURLParameter) {
                        // routing uses camelcase as default, this is why we do a check here again
            if (isset($allParams[$field])) {
                $value = $allParams[$field];
            } elseif (($field !== $camelCaseFieldName) && (isset($allParams[$camelCaseFieldName]))) {
                $value = $allParams[$camelCaseFieldName];
            }
                    } else {
                        if (isset($data[$field])) {
                            $value = $data[$field];
                        } elseif (($field !== $camelCaseFieldName) && (isset($data[$camelCaseFieldName]))) {
            ...
            if (!preg_match("/^[\+\-]*\d+$/", $value)) {
                ...
                $value = (int) $value;
                    ...
                    } elseif (($valueType==='float')||($valueType==='number')) {
                        // float
  ...
  public function checkAccessAction($action)
  {
    ...
    public function queryParametersToSQLFilter($parameters = null, $class = null)
    {
    ...
    public function sendData($data = null, $code = null)
    {
    ...
    public function sendJSON($data = null, $code = null)
    {
        return $this->sendData($data, $code);
    }

    public function sendError($errMsg = 'unspecified error', $errCode = 500)
    {
    ...
    public function sendSuccessfulPut($msg = 'resource updated successfully', $code = 201)
    {
    ...
    public function sendSuccessfulDelete($msg = 'resource deleted successfully', $code = 202)
    {
        $args = $this->sortCodeAndMessage($code, $msg);
        $this->code = $this->statusCode = $args['code'];
        $this->message = $args['message'];
    ...
    public function sendNotFound($msg = 'resource not found', $code = 404)
    {
        $args = $this->sortCodeAndMessage($code, $msg);
        $this->code = $this->statusCode = $args['code'];
        $this->message = $args['message'];
    ...
    public function sendSuccessfulPost($uriOrData = null, $code = 201, $msg = 'resource created succesfully')
    {
        $args = $this->sortCodeAndMessage($code, $msg);
        $this->code = $this->statusCode = $args['code'];
        $this->message = $args['message'];
    ...
    public function sendPermissionFailure($msg = 'permission failure', $code = 401)
    {
        return $this->sendError($msg, $code);
    }

    public function sendParameterError($msg = 'wrong / missing parameter(s)', $code = 422)
    {
        return $this->sendError($msg, $code);
    }

    public function isValidApiSession()
    {
    ...
    public function sendInvalidApiSession()
    {
